...
// CUT begin
	public static void main(String[] args){
		System.err.println("ChessboardPattern (200 Points)");
		System.err.println();
	...
	static void runTest(HashSet<Integer> caseSet) {
	    ...
	    while (true) {
            ...
            if (doTest(rows, columns, __answer))
                passed++;
	    }
	    if (caseSet.size() > 0) cases = caseSet.size();
        System.err.println(String.format("%nPassed : %d/%d cases", passed, cases));

        int T = (int)(System.currentTimeMillis() / 1000) - 1576917329;
        double PT = T / 60.0, TT = 75.0;
        System.err.println(String.format("Time   : %d minutes %d secs%nScore  : %.2f points", T / 60, T % 60, 200 * (0.3 + (0.7 * TT * TT) / (10.0 * PT * PT + TT * TT))));
	}

	static boolean doTest(int rows, int columns, String[] __expected) {
		long startTime = System.currentTimeMillis();
		Throwable exception = null;
		ChessboardPattern instance = new ChessboardPattern();
		String[] __result = new String[0];
		try {
			__result = instance.makeChessboard(rows, columns);
		}
	...
	static class Reader {
        private static final String dataFileName = "ChessboardPattern.sample";
