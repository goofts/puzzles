...
public class ConfusingTriangle {
	public double findMinimum(int L) {
		return 0.0;
	}

// CUT begin
	public static void main(String[] args){
		System.err.println("ConfusingTriangle (400 Points)");
		System.err.println();
	...
	static void runTest(HashSet<Integer> caseSet) {
	    ...
	    while (true) {
            ...
            if (doTest(L, __answer))
                passed++;
	    }
	    if (caseSet.size() > 0) cases = caseSet.size();
        System.err.println(String.format("%nPassed : %d/%d cases", passed, cases));

        int T = (int)(System.currentTimeMillis() / 1000) - 1576917424;
        double PT = T / 60.0, TT = 75.0;
        System.err.println(String.format("Time   : %d minutes %d secs%nScore  : %.2f points", T / 60, T % 60, 400 * (0.3 + (0.7 * TT * TT) / (10.0 * PT * PT + TT * TT))));
	}

	static boolean doTest(int L, double __expected) {
		long startTime = System.currentTimeMillis();
		Throwable exception = null;
		ConfusingTriangle instance = new ConfusingTriangle();
		double __result = 0.0;
		try {
			__result = instance.findMinimum(L);
		}
	...
	static class Reader {
        private static final String dataFileName = "ConfusingTriangle.sample";
