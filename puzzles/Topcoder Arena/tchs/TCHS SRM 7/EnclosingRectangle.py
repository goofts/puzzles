...
def do_test(X, Y, __expected):
    startTime = time.time()
    instance = EnclosingRectangle()
    exception = None
    try:
        __result = instance.smallestArea(X, Y);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("EnclosingRectangle (1000 Points)\n\n")
    ...
    with open("EnclosingRectangle.sample", "r") as f:
        while True:
            label = f.readline()
